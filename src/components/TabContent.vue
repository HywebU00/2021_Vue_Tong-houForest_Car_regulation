...
export default {
  setup() {
    ...
    const NewsArr = reactive([
      {
        name:
          "2-0. 本申請只適用於進出入烏來鄉桶后林道，進入山地特定管制區請另外向烏玉入山檢查所現場辦理入山手續進入山地特定管制區請另外向烏玉入山檢查所現場辦理入山手續進入山地特定管制區請另外向烏玉入山檢查所現場辦理入山手續進入山地特定管制區請另外向烏玉入山檢查所現場辦理入山手續進入山地特定管制區請另外向烏玉入山檢查所現場辦理入山手續。",
        time: "(2021/4/27 3:07:38 AM)",
      },
      {
        name:
          "2-1. 本申請只適用於進出入烏來鄉桶后林道，進入山地特定管制區請另外向烏玉入山檢查所現場辦理入山手續。",
        time: "(2021/4/27 3:07:38 AM)",
      },
      {
      ...
      {
        name: "2-3.[緊急封閉]2010/02/19 ~ 2010/02/26 豪大雨道路中斷",
        time: "(2021/4/27 3:07:38 AM)",
      },
      {
        name: "2-4.[緊急封閉]2010/02/19 ~ 2010/02/26 豪大雨道路中斷",
        time: "(2021/4/27 3:07:38 AM)",
      },
      {
        name: "2-5.[緊急封閉]2010/02/19 ~ 2010/02/26 豪大雨道路中斷",
        time: "(2021/4/27 3:07:38 AM)",
      },
      {
        name:
          "2-6. 本申請只適用於進出入烏來鄉桶后林道，進入山地特定管制區請另外向烏玉入山檢查所現場辦理入山手續。",
        time: "(2021/4/27 3:07:38 AM)",
      },
    ]);
...
<template>
  <div class="tabSet">
    <section class="tabs" :style="{ height: tabContent }">
      <h2
        :class="['tabItem', { active: tagStatus == 'post' }]"
        @click.prevent="tagStatus = 'post'"
      >
        <a href="#">緊急公告</a>
      </h2>
      <!--選到的頁籤項目-->
      <div
        :class="['tabContent', { showContent: tagStatus == 'post' }]"
        :style="{ height: listH }"
      >
        <ul>
          <li v-for="item in filterArr" :key="item.name">
            <a href="#">{{ item.name }}</a
            ><time>(2021/4/27 3:07:38 AM)</time>
          </li>
        </ul>
      </div>
      <h2
        :class="['tabItem', { active: tagStatus == 'news' }]"
        @click.prevent="tagStatus = 'news'"
      >
        <a href="#">最新消息</a>
      </h2>
      <div
        :class="['tabContent', { showContent: tagStatus == 'news' }]"
        :style="{ height: listH }"
      >
        <ul>
          <li v-for="item in filterArr" :key="item.name">
